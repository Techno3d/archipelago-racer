...
func _unhandled_input(event: InputEvent) -> void:
	if event is InputEventMouseMotion && is_mouse_movement_enabled:
		...
		polar_cam.y += motion_event.relative.x/100
		polar_cam.y = clampf(polar_cam.y, original_cam_pivot_polar.y-radian_max.x, original_cam_pivot_polar.y+radian_max.x)
		polar_cam.z += motion_event.relative.y/100
		polar_cam.z = clampf(polar_cam.z, original_cam_pivot_polar.z-radian_max.y, original_cam_pivot_polar.z+radian_max.y)
